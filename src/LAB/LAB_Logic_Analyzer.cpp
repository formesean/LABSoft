...
LAB_Logic_Analyzer::LAB_Logic_Analyzer(LAB& _LAB)
  : LAB_Module(_LAB)
{
...
LAB_Logic_Analyzer::~LAB_Logic_Analyzer() {}

void
LAB_Logic_Analyzer::init_gpio_pins()
{
  for (unsigned chan = 0; chan < (m_parent_data.channel_data.size()); chan++) {
    m_LAB.rpi().gpio.set(
      LABC::PIN::LOGAN[chan], AP::GPIO::FUNC::INPUT, AP::GPIO::PULL::DOWN);
  }
}
...
void
LAB_Logic_Analyzer::config_dma_cb()
{
...
void
LAB_Logic_Analyzer::init_interrupts()
{
...
LAB_Logic_Analyzer::run()
{
  ...
  m_thread_get_samples =
    std::thread(&LAB_Logic_Analyzer::get_samples_loop, this);

  m_parent_data.status = LABE::LOGAN::STATUS::AUTO;
}

void
LAB_Logic_Analyzer::stop()
{
...
bool
LAB_Logic_Analyzer::is_running() const
{
...
void
LAB_Logic_Analyzer::fill_raw_sample_buffer_from_dma_buffer()
{
...
void
LAB_Logic_Analyzer::parse_raw_sample_buffer()
{
...
void
LAB_Logic_Analyzer::get_samples_loop()
{
  LAB_DMA_Data_Logic_Analyzer& dma_data =
    *(static_cast<LAB_DMA_Data_Logic_Analyzer*>(m_uncached_memory.virt()));
  ...
  while (m_parent_data.is_backend_running) {
    ...
    for (unsigned a = 0; a < 1; a++) {
      for (unsigned b = 0; b < LABC::LOGAN::MAX_NUMBER_OF_SAMPLES; b++) {
        dma_data.rxd[a][b] = m_LAB.rpi().gpio.level();
...
LABE::LOGAN::MODE
LAB_Logic_Analyzer::calc_mode(double time_per_division) const
{
...
void
LAB_Logic_Analyzer::set_mode(LABE::LOGAN::MODE mode)
{
...
void
LAB_Logic_Analyzer::dma_buffer_count(LABE::LOGAN::BUFFER_COUNT buffer_count)
{
...
void
LAB_Logic_Analyzer::set_samples(unsigned value)
{
...
void
LAB_Logic_Analyzer::set_time_per_division(double value)
{
...
void
LAB_Logic_Analyzer::set_time_per_division(unsigned samples,
...
double
LAB_Logic_Analyzer::calc_sampling_rate(unsigned samples,
                                       double time_per_division)
{
  double new_sampling_rate =
    samples / (time_per_division * LABC::LOGAN::DISPLAY_NUMBER_OF_COLUMNS);
...
double
LAB_Logic_Analyzer::calc_sample_count(double sampling_rate,
                                      double time_per_division)
{
  double new_sample_count =
    sampling_rate * LABC::LOGAN::DISPLAY_NUMBER_OF_COLUMNS * time_per_division;
...
void
LAB_Logic_Analyzer::set_sampling_rate(double value)
{
  ...
  //     ()));
  //
  //   dma_data.pwm_duty_cycle = (m_LAB.rpi ().pwm.range
  //   (LABC::PWM::DMA_PACING_CHAN)) / 2.0;
  // }
...
void
LAB_Logic_Analyzer::set_trigger_condition(unsigned gpio_pin,
                                          LABE::LOGAN::TRIG::CND condition)
{
  AP::GPIO::EVENT event = AP::GPIO::EVENT::NONE;
...
void
LAB_Logic_Analyzer::parse_trigger_mode()
{
  switch (m_parent_data.trigger_mode) {
    ...
    case (LABE::LOGAN::TRIG::MODE::AUTO): {
      if (!m_thread_find_trigger.joinable()) {
        ...
        m_thread_find_trigger =
          std::thread(&LAB_Logic_Analyzer::find_trigger_point_loop, this);
      }
 ...
 */
void
LAB_Logic_Analyzer::find_trigger_point_loop()
{
  ...
  while (pdata.trigger_enabled) {
    if (!pdata.trigger_found) {
      ...
      if (pdata.trigger_flags != temp_GPSED0) {
        ...
        //    This is to know what buffer (0 or 1) is currently being
        //    filled.
        ...
        if (curr_conblk_ad == buff0_cbs_addr[0] ||
            curr_conblk_ad == buff0_cbs_addr[1]) {
          m_parent_data.trigger_buffer_index = 1;
        } else if (curr_conblk_ad == buff1_cbs_addr[0] ||
                   curr_conblk_ad == buff1_cbs_addr[1]) {
          m_parent_data.trigger_buffer_index = 0;
        }

        // 4. Check if the triggered channel resulted to an actual
        // trigger event.
        //    This takes into account the trigger conditions of other
        //    channels.
      ...
      std::this_thread::sleep_for(std::chrono::duration<double, std::milli>(
        m_parent_data.check_trigger_sleep_period));
    }
  }
}
 ...
 */
bool
LAB_Logic_Analyzer::check_if_triggered(
  uint32_t event_detect_status_register_value)
{
 ...
 */
void
LAB_Logic_Analyzer::create_trigger_frame()
{
  // static constexpr unsigned samp_half           =
  // LABC::LOGAN::MAX_NUMBER_OF_SAMPLES / 2; static constexpr unsigned
  // samp_half_index     = samp_half - 1; static LAB_DMA_Data_Logic_Analyzer&
  // dma_data  = *(static_cast<LAB_DMA_Data_Logic_Analyzer*>
  ...
  //             copy_count_1  = LABC::LOGAN::MAX_NUMBER_OF_SAMPLES - 1 -
  //             m_parent_data.trig_index, copy_count_2  = samp_half -
  //             copy_count_1;
  ...
  //   while (!((*(m_LAB.rpi ().dma.Peripheral::reg (AP::DMA::INT_STATUS)) >>
  //   LABC::DMA::CHAN::OSC_RX) & 0x1));

  //   std::memcpy (
  //     m_parent_data.raw_data_buffer.data () + (copy_count_0 + copy_count_1),
  //     const_cast<const void*>(static_cast<const volatile
  //     void*>(dma_data.rxd[m_parent_data.trigger_buffer_index ^ 1])), sizeof
  //     (uint32_t) * copy_count_2
  //   );

  //   m_LAB.rpi ().dma.Peripheral::reg_wbits (AP::DMA::INT_STATUS, 0,
...
void
LAB_Logic_Analyzer::cache_trigger_condition(unsigned channel,
                                            LABE::LOGAN::TRIG::CND condition)
{
  LABE::LOGAN::TRIG::CND prev_cnd =
    m_parent_data.channel_data[channel].trigger_condition;
      ...
      switch (prev_cnd) {
        ...
        case (LABE::LOGAN::TRIG::CND::EITHER_EDGE): {
          ...
          std::vector<unsigned>::iterator it =
            std::find(vec->begin(), vec->end(), channel);
...
void
LAB_Logic_Analyzer::reset_dma_process()
{
  LAB_DMA_Data_Logic_Analyzer& dma_data =
    *(static_cast<LAB_DMA_Data_Logic_Analyzer*>(m_uncached_memory.virt()));
  ...
  std::memset(const_cast<void*>(static_cast<volatile void*>(dma_data.rxd)),
              0,
              sizeof(dma_data.rxd));
...
void
LAB_Logic_Analyzer::update_data_samples()
{
  if (is_running()) {
    switch (m_parent_data.trigger_mode) {
      case (LABE::LOGAN::TRIG::MODE::NONE): {
...
double
LAB_Logic_Analyzer::calc_samp_count(double time_per_div,
                                    unsigned osc_disp_num_cols)
{
  if (time_per_div >= LABC::LOGAN::MIN_TIME_PER_DIV_NO_ZOOM) {
    return (LABC::LOGAN::MAX_NUMBER_OF_SAMPLES);
  } else {
    return (LABC::LOGAN::MAX_SAMPLING_RATE * osc_disp_num_cols * time_per_div);
  }
}

double
LAB_Logic_Analyzer::calc_samp_rate(double time_per_div,
                                   unsigned osc_disp_num_cols)
{
  if (time_per_div <= LABC::LOGAN::MIN_TIME_PER_DIV_NO_ZOOM) {
    ...
    return (LABC::LOGAN::MAX_NUMBER_OF_SAMPLES /
            (time_per_div * osc_disp_num_cols));
  }
}

LABE::LOGAN::MODE
LAB_Logic_Analyzer::calc_mode(double time_per_division)
{
  LABE::LOGAN::MODE mode;

  if (time_per_division < LABC::LOGAN::MIN_TIME_PER_DIVISION_SCREEN) {
    mode = LABE::LOGAN::MODE::REPEATED;
  } else if (m_parent_data.time_per_division <
               LABC::LOGAN::MIN_TIME_PER_DIVISION_SCREEN &&
             time_per_division >= LABC::LOGAN::MIN_TIME_PER_DIVISION_SCREEN) {
    mode = LABE::LOGAN::MODE::SCREEN;
  } else {
    mode = LABE::LOGAN::MODE::SCREEN; // not sure
  }

  return (mode);
}

LAB_Parent_Data_Logic_Analyzer&
LAB_Logic_Analyzer::parent_data()
{
  return (m_parent_data);
}

void
LAB_Logic_Analyzer::mode(LABE::LOGAN::MODE mode)
{
  switch (mode) {
    case (LABE::LOGAN::MODE::REPEATED): {
      break;
...
void
LAB_Logic_Analyzer::horizontal_offset(double value)
{
  m_parent_data.horizontal_offset = value;
}

double
LAB_Logic_Analyzer::horizontal_offset() const
{
  return (m_parent_data.horizontal_offset);
}

void
LAB_Logic_Analyzer::time_per_division(double value)
{
  if (LABF::is_within_range(value,
                            LABC::LOGAN::MIN_TIME_PER_DIVISION,
    ...
    if (value < LABC::LOGAN::MIN_TIME_PER_DIVISION_NO_ZOOM) {
      unsigned new_sample_count = static_cast<unsigned>(
        calc_sample_count(m_parent_data.sampling_rate, value));
...
double
LAB_Logic_Analyzer::time_per_division() const
{
  return (m_parent_data.time_per_division);
}

void
LAB_Logic_Analyzer::samples(unsigned value)
{
}

unsigned
LAB_Logic_Analyzer::samples() const
{
  return (m_parent_data.samples);
}

void
LAB_Logic_Analyzer::sampling_rate(double value)
{
  if (LABF::is_within_range(value,
                            LABC::LOGAN::MIN_SAMPLING_RATE,
...
double
LAB_Logic_Analyzer::calc_time_per_division(unsigned samples,
                                           double sampling_rate)
{
...
double
LAB_Logic_Analyzer::sampling_rate() const
{
  return (m_parent_data.sampling_rate);
}

void
LAB_Logic_Analyzer::trigger_mode(LABE::LOGAN::TRIG::MODE value)
{
...
void
LAB_Logic_Analyzer::trigger_condition(unsigned channel,
                                      LABE::LOGAN::TRIG::CND condition)
{

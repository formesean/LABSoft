...
LAB_LABChecker_Digital::LAB_LABChecker_Digital() {}

std::stringstream
LAB_LABChecker_Digital::create_circuit_checker_xml_stringstream(
  ...
  const std::vector<std::vector<char>>& outputs)
{
  ...
  for (std::size_t row = 0; row < inputs.size(); row++) {
    ...
    input_node.append_child(pugi::node_pcdata)
      .set_value(std::string(inputs[row].begin(), inputs[row].end()).c_str());

    // 3. output
    pugi::xml_node output_node = data_pair.append_child("output");

    output_node.append_child(pugi::node_pcdata)
      .set_value(std::string(outputs[row].begin(), outputs[row].end()).c_str());
  }
...
void
LAB_LABChecker_Digital::encrypt_stringstream(std::stringstream& ss,
...
void
LAB_LABChecker_Digital::save_stringstream_to_file(const std::stringstream& ss,
...
void
LAB_LABChecker_Digital::create_circuit_checker_file(
  const std::vector<std::vector<char>>& inputs,
  const std::vector<std::vector<char>>& outputs,
  const std::string& file_path,
  const char* password)
{
  std::stringstream ss =
    create_circuit_checker_xml_stringstream(inputs, outputs);

...
{
  value ? rpi().pwm.start(LABC::PWM::DMA_PACING_CHAN) : rpi().pwm.stop(LABC::PWM::DMA_PACING_CHAN);
                                  ...
                                  unsigned channel)
{
  ...
  double actual_value = sign ? actual_value_absolute : (actual_value_absolute - cdata.calibration.conversion_reference_voltage);
    ...
    find_trigger_point()
{
  ...
  {
    for (int a = 0; a < m_parent_data.trig_buffs.pre_trigger[m_parent_data.trigger_buffer_index].size(); a += m_parent_data.find_trig_sample_skip)
    {
    ...
    create_trigger_frame()
{
  ...
  static LAB_DMA_Data_Oscilloscope &dma_data = *(static_cast<LAB_DMA_Data_Oscilloscope *>(m_uncached_memory.virt()));
  ...
  if (m_parent_data.trig_index >= samp_half_index)
  {
    ...
    std::memcpy(
        m_parent_data.raw_data_buffer.data(),
        m_parent_data.trig_buffs.pre_trigger[m_parent_data.trigger_buffer_index].data() + (m_parent_data.trig_index - samp_half_index),
        sizeof(uint32_t) * copy_count_0);

    std::memcpy(
        m_parent_data.raw_data_buffer.data() + (copy_count_0),
        m_parent_data.trig_buffs.pre_trigger[m_parent_data.trigger_buffer_index].data() + (m_parent_data.trig_index + 1),
  ...
  else if (m_parent_data.trig_index < samp_half_index)
  {
    ...
    std::memcpy(
        m_parent_data.raw_data_buffer.data(),
        m_parent_data.trig_buffs.pre_trigger[m_parent_data.trigger_buffer_index ^ 1].data() + (LABC::OSC::NUMBER_OF_SAMPLES - copy_count_0),
        sizeof(uint32_t) * copy_count_0);
...
void LAB_Oscilloscope::Calibration::
    vertical_offset_corrector(unsigned channel,
                              ...
                              double value)
{
  m_pdata.channel_data.at(channel).calibration.vertical_offset_corrector.at(scaling) = value;
}
...
void LAB_Oscilloscope::Calibration::
    scaling_corrector_to_unity(unsigned channel,
                               ...
                               double value)
{
  m_pdata.channel_data.at(channel).calibration.scaling_corrector_to_unity.at(scaling) = value;
}
...
void LAB_Oscilloscope::Calibration::
    scaling_corrector_to_actual(unsigned channel,
                                ...
                                double value)
{
  m_pdata.channel_data.at(channel).calibration.scaling_corrector_to_actual.at(scaling) = value;
}
...
{
  return (m_pdata.channel_data.at(channel).calibration.vertical_offset_corrector.at(scaling));
}
...
double LAB_Oscilloscope::Calibration::
    scaling_corrector_to_unity(unsigned channel,
                               LABE::OSC::SCALING scaling) const
{
  return (m_pdata.channel_data.at(channel).calibration.scaling_corrector_to_unity.at(scaling));
}
...
double LAB_Oscilloscope::Calibration::
    scaling_corrector_to_actual(unsigned channel,
                                LABE::OSC::SCALING scaling) const
{
  return (m_pdata.channel_data.at(channel).calibration.scaling_corrector_to_actual.at(scaling));
}

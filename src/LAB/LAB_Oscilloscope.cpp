...
LAB_Oscilloscope::LAB_Oscilloscope(LAB& _LAB)
  : LAB_Module(_LAB)
...
LAB_Oscilloscope::init_spi()
{
  ...
  rpi().gpio.set(
    LABC::PIN::OSC::ADC_CS, AP::GPIO::FUNC::ALT0, AP::GPIO::PULL::OFF);
  rpi().gpio.set(
    LABC::PIN::OSC::ADC_MISO, AP::GPIO::FUNC::ALT0, AP::GPIO::PULL::DOWN);
  rpi().gpio.set(
    LABC::PIN::OSC::ADC_MOSI, AP::GPIO::FUNC::ALT0, AP::GPIO::PULL::OFF);
  rpi().gpio.set(
    LABC::PIN::OSC::ADC_SCLK, AP::GPIO::FUNC::ALT0, AP::GPIO::PULL::OFF);
}

void
LAB_Oscilloscope::init_pwm()
{
  ...
  // rpi ().gpio.set (LABC::PIN::PWM, AP::GPIO::FUNC::ALT0,
  // AP::GPIO::PULL::DOWN);
}

void
LAB_Oscilloscope::init_gpio_pins()
{
  // scaling
  for (std::size_t chan = 0; chan < m_parent_data.channel_data.size(); chan++) {
    for (int a = 0; a < 2; a++) {
...
void
LAB_Oscilloscope::init_dma()
{
  config_dma_cb();

  AikaPi::Uncached& un = m_uncached_memory;
  LAB_DMA_Data_Oscilloscope& dd =
    *(static_cast<LAB_DMA_Data_Oscilloscope*>(un.virt()));
...
void
LAB_Oscilloscope::init_state()
{
  for (std::size_t a = 0; a < m_parent_data.channel_data.size(); a++) {
    scaling(static_cast<unsigned>(a), m_parent_data.channel_data[a].scaling);

    coupling(static_cast<unsigned>(a), m_parent_data.channel_data[a].coupling);
  }
...
void
LAB_Oscilloscope::config_dma_cb()
{
  ...
  LAB_DMA_Data_Oscilloscope new_uncached_dma_data{}; // default initialize

  new_uncached_dma_data.cbs[0] = {
    LABC::DMA::TI::OSC_RX,
  ...
  new_uncached_dma_data.spi_samp_size = sizeof(uint32_t);
  new_uncached_dma_data.spi_cs = (1 << 7) | (1 << 11) | (1 << 8) | (0 << 3) |
                                 (0 << 2) | LABC::OSC::ADC_SPI_CHIP_ENABLE;
  new_uncached_dma_data.spi_cs_fifo_reset = 0x00000030;
  new_uncached_dma_data.pwm_duty_cycle = 0x0;
  new_uncached_dma_data.txd = 0x00000000ffff;

  std::memcpy(
    &uncached_dma_data, &new_uncached_dma_data, sizeof(new_uncached_dma_data));
}
void
LAB_Oscilloscope::run()
{
...
void
LAB_Oscilloscope::frontend_run_stop(bool value)
{
  m_parent_data.status =
    value ? LABE::OSC::STATUS::AUTO : LABE::OSC::STATUS::STOP;

  m_parent_data.is_frontend_running = value;
}

void
LAB_Oscilloscope::backend_run_stop(bool value)
{
...
void
LAB_Oscilloscope::do_measurements(bool value)
{
  m_parent_data.do_measurements = value;
}

void
LAB_Oscilloscope::status(LABE::OSC::STATUS _STATUS)
{
  m_parent_data.status = _STATUS;
}

void
LAB_Oscilloscope::reload_settings()
{
  ...
  for (std::size_t a = 0; a < m_parent_data.channel_data.size(); a++) {
    ...
    lab().m_Oscilloscope.calibration().scaling_corrector_to_actual(
      ...
      calibration().scaling_corrector_to_actual(static_cast<unsigned>(a),
                                                LABC::VOLTMETER::SCALING));
...
bool
LAB_Oscilloscope::has_enabled_channels() const
{
  return (m_parent_data.has_enabled_channels());
}

void
LAB_Oscilloscope::channel_enable_disable(unsigned channel, bool enable)
{
...
void
LAB_Oscilloscope::voltage_per_division(unsigned channel, double value)
{
  if (LABF::is_within_range(value,
...
void
LAB_Oscilloscope::vertical_offset(unsigned channel, double value)
{
  m_parent_data.channel_data[channel].vertical_offset = value;
}

void
LAB_Oscilloscope::coupling(unsigned channel, LABE::OSC::COUPLING coupling)
{
  if (channel < m_parent_data.channel_data.size()) {
    switch (coupling) {
      case (LABE::OSC::COUPLING::DC): {
        rpi().gpio.set(LABC::PIN::OSC::RELAY[channel],
                       AP::GPIO::FUNC::OUTPUT,
                       AP::GPIO::PULL::OFF,
                       0);
...
void
LAB_Oscilloscope::scaling(unsigned channel, LABE::OSC::SCALING scaling)
{
  int mux_a = 0;
  ...
  for (int a = 0; a < 2; a++) {
    rpi().gpio.write(LABC::PIN::OSC::MUX[channel][a], (mux_a >> a) & 0x1);
  }
...
LABE::OSC::SCALING
LAB_Oscilloscope::scaling(unsigned channel) const
{
  return (m_parent_data.channel_data[channel].scaling);
}

std::size_t
LAB_Oscilloscope::channels() const
{
  return (m_parent_data.channel_data.size());
}

LABE::OSC::COUPLING
LAB_Oscilloscope::coupling(unsigned channel)
{
  return (m_parent_data.channel_data[channel].coupling);
}

void
LAB_Oscilloscope::update_data_samples()
{
...
void
LAB_Oscilloscope::fill_raw_osc_samp_buff_from_dma_buff()
{
    ...
    case (LABE::OSC::MODE::REPEATED): {
      for (int buff = 0; buff < 2; buff++) {
        if (dma_data.status[buff]) {
          std::memcpy(m_parent_data.raw_data_buffer.data(),
                      const_cast<const void*>(
                        static_cast<volatile void*>(dma_data.rxd[buff])),
...
void
LAB_Oscilloscope::parse_raw_osc_samp_buff()
{
  ...
  for (unsigned samp = 0; samp < pdata.samples; samp++) {
    ...
    //   <<
    //   "\n"; std::cout << "chan 1 adc              : " << std::bitset<12>
    //   (reconstruct_adc_data_from_raw_osc_chan_samp (chan1, 0)) << "\n";
    //   std::cout << "chan 2 adc              : " << std::bitset<12>
    //   (reconstruct_adc_data_from_raw_osc_chan_samp (chan2, 1)) << "\n";
    //   std::cout << "act chan 1              : " <<
    //   conv_raw_osc_samp_to_actual_chan_value (rsamp, 0) << "\n"; std::cout
    //   << "act chan 2              : " <<
    //   conv_raw_osc_samp_to_actual_chan_value (rsamp, 1) << "\n";
    ...
    for (chan = 0; chan < pdata.channel_data.size(); chan++) {
      if (!is_channel_enabled(static_cast<unsigned>(chan))) {
        continue;
      }

      pdata.channel_data[chan].samples[samp] =
        conv_raw_osc_samp_to_actual_chan_value(pdata.raw_data_buffer[samp],
                                               static_cast<unsigned>(chan));

      // FOR DEBUG TO GENERATE DUMMY SINE DATA
      // SINE
      // double y = (0.5 * sin (2 * 3.141592653 * (1.0 / 0.005) * (samp /
      // 16'000.0))) + 0.001;
      ...
      // pdata.channel_data[chan].samples[samp] =  (y *
      // m_parent_data.channel_data[chan].scaling_corrector) + 0.322;
    }
  }

  if (pdata.do_measurements) {
    for (std::size_t chan = 0; chan < pdata.channel_data.size(); chan++) {
      if (is_channel_enabled(static_cast<unsigned>(chan))) {
        auto [min, max, avg] =
          LAB_Data_Measurer::min_max_avg(pdata.channel_data[chan].samples);

        pdata.channel_data[chan].measurements.min = min;
        pdata.channel_data[chan].measurements.max = max;
        pdata.channel_data[chan].measurements.avg = avg;
        pdata.channel_data[chan].measurements.trms =
          LAB_Data_Measurer::true_rms<double>(pdata.channel_data[chan].samples);

        // std::cout << "chan " << chan << "min: " <<
        // pdata.channel_data[chan].measurements.min   << "\n"; std::cout
        // << "chan " << chan << "max: " <<
        // pdata.channel_data[chan].measurements.max   << "\n"; std::cout
        // << "chan " << chan << "avg: " <<
        // pdata.channel_data[chan].measurements.avg   << "\n"; std::cout
        // << "chan " << chan << "trms: " <<
        // pdata.channel_data[chan].measurements.trms  << "\n";
      }
    }
...
double
LAB_Oscilloscope::conv_raw_osc_samp_to_actual_chan_value(uint32_t raw_osc_samp,
                                                         unsigned channel)
{
  uint32_t raw_osc_chan_samp =
    extract_raw_osc_chan_samp_from_raw_osc_samp(raw_osc_samp, channel);
  uint32_t adc_data =
    reconstruct_adc_data_from_raw_osc_chan_samp(raw_osc_chan_samp, channel);
...
uint32_t
LAB_Oscilloscope::extract_raw_osc_chan_samp_from_raw_osc_samp(
  uint32_t raw_osc_samp,
  unsigned channel)
{
...
uint32_t
LAB_Oscilloscope::reconstruct_adc_data_from_raw_osc_chan_samp(
  uint32_t raw_osc_chan_samp,
  unsigned channel)
{
  // MCP33111
  // uint32_t recon_adc_data =    ((raw_osc_chan_samp & 0x007F) << 5) |
  // ((raw_osc_chan_samp & 0xF800) >> 11);
  //          recon_adc_data <<=  1;

  // ADS7883
  uint32_t recon_adc_data =
    ((raw_osc_chan_samp & 0xFE00) >> 9) | ((raw_osc_chan_samp & 0x001F) << 7);

  return (recon_adc_data);
}

uint32_t
LAB_Oscilloscope::reconstruct_raw_osc_chan_samp_from_adc_data(uint32_t adc_data)
{
  // MCP33111
  //          adc_data                >>= 1;
  // uint32_t recon_raw_osc_chan_samp =   ((adc_data & 0x0FE0) >> 5) |
  // ((adc_data & 0x001F) << 11);

  // ADS7883
  uint32_t recon_raw_osc_chan_samp =
    ((adc_data & 0x007F) << 9) | ((adc_data & 0x0F800) >> 7);

  return (recon_raw_osc_chan_samp);
}

double
LAB_Oscilloscope::conv_adc_data_to_actual_value(uint32_t adc_data,
                                                unsigned channel)
{
  ...
  uint32_t adc_data_without_sign =
    adc_data & ((LABC::OSC::ADC_RESOLUTION_INT - 1) >> 1);
  double actual_value_absolute =
    adc_data_without_sign * cdata.calibration.conversion_constant;
  double actual_value = sign ? actual_value_absolute
                             : (actual_value_absolute -
                                cdata.calibration.conversion_reference_voltage);

  if (m_parent_data.is_calibration_enabled) {
    actual_value +=
      cdata.calibration.vertical_offset_corrector.at(cdata.scaling);
    actual_value *=
      cdata.calibration.scaling_corrector_to_unity.at(cdata.scaling);
    actual_value *=
      cdata.calibration.scaling_corrector_to_actual.at(cdata.scaling);
  }
...
uint32_t
LAB_Oscilloscope::conv_raw_osc_samp_to_recon_chan_adc_data(
  uint32_t raw_osc_samp,
  unsigned channel)
{
  uint32_t raw_osc_chan_samp =
...
void
LAB_Oscilloscope::reset_dma_process()
{
  LAB_DMA_Data_Oscilloscope& dma_data =
    *(static_cast<LAB_DMA_Data_Oscilloscope*>(m_uncached_memory.virt()));
  ...
  // 2. reset the DMA engine to the first control block, depending on the
  // buffer
  switch (mode()) {
...
void
LAB_Oscilloscope::reset_uncached_rx_buffer()
{
  LAB_DMA_Data_Oscilloscope& dma_data =
    *(static_cast<LAB_DMA_Data_Oscilloscope*>(m_uncached_memory.virt()));
  ...
  //   reconstruct_raw_osc_chan_samp_from_adc_data
  //   (LABC::OSC::ADC_RESOLUTION_INT / 2), sizeof (dma_data.rxd)
  // );
}

void
LAB_Oscilloscope::record_raw_sample_buffer_metadata()
{
...
double
LAB_Oscilloscope::calc_time_per_division(unsigned samples,
...
double
LAB_Oscilloscope::calc_sampling_rate(unsigned samples,
                                     double time_per_division) const
{
  double new_sampling_rate =
    samples / (time_per_division * LABC::OSC::DISPLAY_NUMBER_OF_COLUMNS);

  if (LABF::is_greater_than(new_sampling_rate,
...
void
LAB_Oscilloscope::set_time_per_division(double value)
{
...
void
LAB_Oscilloscope::set_samples(unsigned value)
{
  ...
  uncached_dma_data.cbs[0].txfr_len =
    static_cast<uint32_t>(sizeof(uint32_t) * value);
  uncached_dma_data.cbs[2].txfr_len =
    static_cast<uint32_t>(sizeof(uint32_t) * value);
  uncached_dma_data.cbs[4].txfr_len =
    static_cast<uint32_t>(sizeof(uint32_t) * value);
}

void
LAB_Oscilloscope::set_sampling_rate(double value)
{
  ...
  dma_data.pwm_duty_cycle =
    static_cast<uint32_t>((rpi().pwm.range(LABC::PWM::DMA_PACING_CHAN)) / 2.0);
...
LABE::OSC::MODE
LAB_Oscilloscope::calc_mode(double time_per_division)
{
  if (LABF::is_less_than(time_per_division,
...
void
LAB_Oscilloscope::horizontal_offset(double value)
{
  if (LABF::is_within_range(value,
...
double
LAB_Oscilloscope::horizontal_offset() const
{
  return (m_parent_data.horizontal_offset);
}

void
LAB_Oscilloscope::time_per_division(double value)
{
  if (LABF::is_within_range(value,
...
void
LAB_Oscilloscope::samples(unsigned value)
{
  if (!(LABF::is_within_range(value,
                              LABC::OSC::MIN_SAMPLES,
...
void
LAB_Oscilloscope::sampling_rate(double value)
{
  if (LABF::is_within_range(value,
...
void
LAB_Oscilloscope::parse_trigger_mode()
{
  switch (m_parent_data.trigger_mode) {
    case (LABE::OSC::TRIG::MODE::NONE): {
    ...
    case (LABE::OSC::TRIG::MODE::AUTO): {
      if (!m_thread_trigger.joinable()) {
        ...
        m_thread_trigger =
          std::thread(&LAB_Oscilloscope::find_trigger_point_loop, this);
      }
...
void
LAB_Oscilloscope::trigger_osc_mode_dispatcher()
{
  // switch (m_parent_data.mode)
  // {
  //   case (LABE::OSC::MODE::REPEATED):
...
inline void
LAB_Oscilloscope::find_trigger_point_loop()
{
  LAB_DMA_Data_Oscilloscope& dma_data =
    *(static_cast<LAB_DMA_Data_Oscilloscope*>(m_uncached_memory.virt()));

  // Cached variables
  volatile uint32_t* curr_conblk_ad =
    rpi().dma.reg(LABC::DMA::CHAN::OSC_RX, AP::DMA::CONBLK_AD);
  ...
  switch (m_parent_data.mode) {
    case (LABE::OSC::MODE::REPEATED): {
      ...
      rpi().dma.reg(LABC::DMA::CHAN::OSC_RX,
                    AP::DMA::NEXTCONBK,
                    m_uncached_memory.bus(&dma_data.cbs[0]));
      rpi().dma.reg_wbits(LABC::DMA::CHAN::OSC_RX, AP::DMA::CS, 1, 30);
      std::this_thread::sleep_for(std::chrono::microseconds(5));
      rpi().dma.run(LABC::DMA::CHAN::OSC_RX);
      while (*curr_conblk_ad == buff0_cbs_addr[0] ||
             *curr_conblk_ad == buff0_cbs_addr[1])
        ;
      while (*curr_conblk_ad == buff1_cbs_addr[0] ||
             *curr_conblk_ad == buff1_cbs_addr[1])
        ;
      rpi().dma.MemoryMap::reg_wbits(
        AP::DMA::INT_STATUS, 0, LABC::DMA::CHAN::OSC_RX);
      break;
    }
    case (LABE::OSC::MODE::SCREEN): {
      break;
  ...
  while (m_parent_data.trigger_enabled) {
    if (!m_parent_data.trigger_found) {
      if (rpi().dma.interrupt(LABC::DMA::CHAN::OSC_RX)) {
        uint32_t current_cb_addr =
          *(rpi().dma.reg(LABC::DMA::CHAN::OSC_RX, AP::DMA::CONBLK_AD));

        rpi().dma.clear_interrupt(LABC::DMA::CHAN::OSC_RX);

        std::memcpy(m_parent_data.trig_buffs.pre_trigger.data(),
                    const_cast<const void*>(
                      static_cast<const volatile void*>(dma_data.rxd)),
                    sizeof(uint32_t) * LABC::OSC::NUMBER_OF_CHANNELS *
                      LABC::OSC::NUMBER_OF_SAMPLES);

        if (current_cb_addr == buff0_cbs_addr[0] ||
            current_cb_addr == buff0_cbs_addr[1]) {
          m_parent_data.trigger_buffer_index = 1;
        } else if (current_cb_addr == buff1_cbs_addr[0] ||
                   current_cb_addr == buff1_cbs_addr[1]) {
...
inline bool
LAB_Oscilloscope::find_trigger_point()
{
  switch (m_parent_data.trig_type) {
    case (LABE::OSC::TRIG::TYPE::LEVEL): {
      for (std::size_t a = 0;
           a < m_parent_data.trig_buffs
                 .pre_trigger[m_parent_data.trigger_buffer_index]
                 .size();
           a += m_parent_data.find_trig_sample_skip) {
        uint32_t samp = conv_raw_osc_samp_to_recon_chan_adc_data(
          m_parent_data.trig_buffs
            .pre_trigger[m_parent_data.trigger_buffer_index][a],
          m_parent_data.trigger_source);

        if (samp >= m_parent_data.trigger_level_raw_bits) {
          m_parent_data.trig_index = static_cast<unsigned>(a);
    ...
    case (LABE::OSC::TRIG::TYPE::EDGE): {
      uint32_t prev = conv_raw_osc_samp_to_recon_chan_adc_data(
        m_parent_data.trig_buffs
          .pre_trigger[m_parent_data.trigger_buffer_index][0],
        m_parent_data.trigger_source);
      ...
      switch (m_parent_data.trig_condition) {
        ...
        case (LABE::OSC::TRIG::CND::EITHER): {
          for (std::size_t a = 1;
            ...
            if (((samp <= m_parent_data.trigger_level_raw_bits) &&
                 (prev > m_parent_data.trigger_level_raw_bits)) ||
                ((samp >= m_parent_data.trigger_level_raw_bits) &&
                 (prev < m_parent_data.trigger_level_raw_bits))) {
              m_parent_data.trig_index = static_cast<unsigned>(a);
              m_parent_data.trigger_found = true;
...
LAB_Oscilloscope::create_trigger_frame()
{
  ...
  if (m_parent_data.trig_index >= samp_half_index) {
    ...
    while (!((*(rpi().dma.MemoryMap::reg(AP::DMA::INT_STATUS)) >>
              LABC::DMA::CHAN::OSC_RX) &
             0x1))
      ;

    std::memcpy(m_parent_data.raw_data_buffer.data() +
                  (copy_count_0 + copy_count_1),
                const_cast<const void*>(static_cast<const volatile void*>(
                  dma_data.rxd[m_parent_data.trigger_buffer_index ^ 1])),
    ...
    std::memcpy(m_parent_data.raw_data_buffer.data(),
                m_parent_data.trig_buffs
                    .pre_trigger[m_parent_data.trigger_buffer_index ^ 1]
                    .data() +
...
void
LAB_Oscilloscope::find_trigger_timeout_timer()
{
...
uint32_t
LAB_Oscilloscope::calc_trigger_level_raw_bits(double trigger_level)
{
  if (LABF::is_within_range(trigger_level,
                            LABD::OSC::MIN_OSC_HARDWARE_TRIGGER_LEVEL,
                            LABD::OSC::MAX_OSC_HARDWARE_TRIGGER_LEVEL,
                            LABC::LABSOFT::EPSILON)) {
    return (reconstruct_raw_osc_chan_samp_from_adc_data(
      static_cast<uint32_t>(std::round(trigger_level))));
  } else {
    return (0xFFFFFFFF);
  }
}
...
void
LAB_Oscilloscope::prefill_buffers_for_triggering()
{
  LAB_DMA_Data_Oscilloscope& dma_data =
    *(static_cast<LAB_DMA_Data_Oscilloscope*>(m_uncached_memory.virt()));
...
void
LAB_Oscilloscope::config_dma_cb_record()
{
  // LAB_DMA_Data_Oscilloscope dma_data =
  // {
  //   .cbs =
...
void
LAB_Oscilloscope::trigger_mode(LABE::OSC::TRIG::MODE value)
{
...
LABE::OSC::TRIG::MODE
LAB_Oscilloscope::trigger_mode() const
{
  return (m_parent_data.trigger_mode);
}

void
LAB_Oscilloscope::trigger_source(unsigned chan)
{
  if (chan <= LABC::OSC::NUMBER_OF_CHANNELS) {
    m_parent_data.trigger_source = chan;
  }
}

unsigned
LAB_Oscilloscope::trigger_source() const
{
  return (m_parent_data.trigger_source);
}

void
LAB_Oscilloscope::trigger_type(LABE::OSC::TRIG::TYPE value)
{
  m_parent_data.trig_type = value;
}

LABE::OSC::TRIG::TYPE
LAB_Oscilloscope::trigger_type() const
{
  return (m_parent_data.trig_type);
}

void
LAB_Oscilloscope::trigger_condition(LABE::OSC::TRIG::CND value)
{
  m_parent_data.trig_condition = value;
}

LABE::OSC::TRIG::CND
LAB_Oscilloscope::trigger_condition() const
{
  return (m_parent_data.trig_condition);
}

void
LAB_Oscilloscope::trigger_level(double value)
{
  if (LABF::is_within_range(value,
...
double
LAB_Oscilloscope::trigger_level() const
{
  return (m_parent_data.trigger_level);
}

bool
LAB_Oscilloscope::trigger_found() const
{
  return (m_parent_data.trigger_found);
}

void
LAB_Oscilloscope::trigger_serviced()
{
  m_parent_data.trigger_found = false;
}

void
LAB_Oscilloscope::record_start()
{
  // record_init ();
...
void
LAB_Oscilloscope::record_init()
{
  // x.
  m_parent_data.recording_raw_sample_buffer.reserve(
    LABC::OSC::MAX_SAMPLES_RECORDING);
}
...
void
LAB_Oscilloscope::mode(LABE::OSC::MODE mode)
{
  switch (mode) {
    case (LABE::OSC::MODE::REPEATED): {
      break;
...
LABE::OSC::MODE
LAB_Oscilloscope::mode()
{
  return (m_parent_data.mode);
}

void
LAB_Oscilloscope::set_mode(LABE::OSC::MODE mode)
{
...
void
LAB_Oscilloscope::dma_buffer_count(LABE::OSC::BUFFER_COUNT buffer_count)
{
...
bool
LAB_Oscilloscope::is_channel_enabled(unsigned channel) const
{
...
// Getters
bool
LAB_Oscilloscope::is_frontend_running() const
{
  return (m_parent_data.is_frontend_running);
}

bool
LAB_Oscilloscope::is_backend_running() const
{
  return (m_parent_data.is_backend_running);
}

LABE::OSC::STATUS
LAB_Oscilloscope::status() const
{
  return (m_parent_data.status);
}

double
LAB_Oscilloscope::voltage_per_division(unsigned channel) const
{
  return (m_parent_data.channel_data[channel].voltage_per_division);
}

double
LAB_Oscilloscope::vertical_offset(unsigned channel) const
{
  return (m_parent_data.channel_data[channel].vertical_offset);
}

double
LAB_Oscilloscope::time_per_division() const
{
  return (m_parent_data.time_per_division);
}

unsigned
LAB_Oscilloscope::samples() const
{
  return (m_parent_data.samples);
}

double
LAB_Oscilloscope::sampling_rate() const
{
  return (m_parent_data.sampling_rate);
}

const LAB_Parent_Data_Oscilloscope&
LAB_Oscilloscope::parent_data()
{
  return (m_parent_data);
}

const std::array<double, LABC::OSC::NUMBER_OF_SAMPLES>&
LAB_Oscilloscope::chan_samples(unsigned channel) const
{
  return (m_parent_data.channel_data[channel].samples);
}

double
LAB_Oscilloscope::chan_voltage_per_division(unsigned channel) const
{
  return (m_parent_data.channel_data[channel].voltage_per_division);
}

double
LAB_Oscilloscope::chan_vertical_offset(unsigned channel) const
{
  return (m_parent_data.channel_data[channel].vertical_offset);
}

double
LAB_Oscilloscope::raw_buffer_time_per_division() const
{
  return (m_parent_data.time_per_division_raw_buffer);
}

double
LAB_Oscilloscope::raw_buffer_horizontal_offset() const
{
  return (m_parent_data.horizontal_offset_raw_buffer);
}

double
LAB_Oscilloscope::raw_buffer_sampling_rate() const
{
...
LAB_Oscilloscope::Measurements::Measurements(
  LAB_Parent_Data_Oscilloscope& _LAB_Parent_Data_Oscilloscope)
  : m_pdata(_LAB_Parent_Data_Oscilloscope)
{
}

double
LAB_Oscilloscope::Measurements::avg(unsigned channel)
{
  return (m_pdata.channel_data[channel].measurements.avg);
}

double
LAB_Oscilloscope::Measurements::max(unsigned channel)
{
  return (m_pdata.channel_data[channel].measurements.max);
}

double
LAB_Oscilloscope::Measurements::min(unsigned channel)
{
  return (m_pdata.channel_data[channel].measurements.min);
}

double
LAB_Oscilloscope::Measurements::trms(unsigned channel)
{
...
LAB_Oscilloscope::Calibration::Calibration(
  LAB_Parent_Data_Oscilloscope& _LAB_Parent_Data_Oscilloscope)
  : m_pdata(_LAB_Parent_Data_Oscilloscope)
{
}
...
void
LAB_Oscilloscope::Calibration::enable()
{
  m_pdata.is_calibration_enabled = true;
}

void
LAB_Oscilloscope::Calibration::disable()
{
  m_pdata.is_calibration_enabled = false;
}

bool
LAB_Oscilloscope::Calibration::is_enabled() const
{
  return (m_pdata.is_calibration_enabled);
}

void
LAB_Oscilloscope::Calibration::adc_reference_voltage(unsigned channel,
                                                     double value)
{
  LAB_Calibration_Data_Oscilloscope& calib =
    m_pdata.channel_data[channel].calibration;
...
void
LAB_Oscilloscope::Calibration::vertical_offset_corrector(
  unsigned channel,
  LABE::OSC::SCALING scaling,
  double value)
{
  m_pdata.channel_data.at(channel).calibration.vertical_offset_corrector.at(
    scaling) = value;
}

void
LAB_Oscilloscope::Calibration::scaling_corrector_to_unity(
  unsigned channel,
  LABE::OSC::SCALING scaling,
  double value)
{
  m_pdata.channel_data.at(channel).calibration.scaling_corrector_to_unity.at(
    scaling) = value;
}

void
LAB_Oscilloscope::Calibration::scaling_corrector_to_actual(
  unsigned channel,
  LABE::OSC::SCALING scaling,
  double value)
{
  m_pdata.channel_data.at(channel).calibration.scaling_corrector_to_actual.at(
    scaling) = value;
}

double
LAB_Oscilloscope::Calibration::adc_reference_voltage(unsigned channel) const
{

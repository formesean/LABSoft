...
constexpr int CHANNEL_INFO_WIDTH = CHANNEL_DRAGGER_WIDTH + CHANNEL_NAME_WIDTH
                                   + CHANNEL_SETTING_WIDTH + CHANNEL_DIO_WIDTH
...
constexpr double CHANNEL_GRAPH_PEAK_TO_PEAK_SPREAD
    = 60.0; // in percent, with the graph widget height as max
...
public:
  LABSoft_GUI_Logic_Analyzer_Display_Channel_Graph (int X, int Y, int W, int H,
                                                    const char *label = 0);
  ~LABSoft_GUI_Logic_Analyzer_Display_Channel_Graph ();
...
public:
  LABSoft_GUI_Logic_Analyzer_Display_Channel_Widget (int X, int Y, int W,
                                                     int H,
                                                     const char *label = 0);
  ~LABSoft_GUI_Logic_Analyzer_Display_Channel_Widget ();
...
public:
  LABSoft_GUI_Logic_Analyzer_Display_Graph_Overlay (int X, int Y, int W, int H,
                                                    const char *label = 0);
  ~LABSoft_GUI_Logic_Analyzer_Display_Graph_Overlay ();
...
private:
  ...
  std::array<Fl_Box *, LABC::LOGAN::DISPLAY_NUMBER_OF_COLUMNS>
...
private:
  ...
  void calc_pp_coords (bool curr_samp, bool next_samp, int next_x,
                       int curr_index,
...
public:
  LABSoft_GUI_Logic_Analyzer_Display (int X, int Y, int W, int H,
                                      const char *label = 0);
  ~LABSoft_GUI_Logic_Analyzer_Display ();

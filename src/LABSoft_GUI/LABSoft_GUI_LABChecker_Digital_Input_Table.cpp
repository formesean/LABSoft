...
{
  when(static_cast<unsigned char>(FL_WHEN_NOT_CHANGED | when()));
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::init_table_dimensions()
{
  row_header(1);
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::create_input_widget(unsigned W,
                                                                unsigned H)
{
  ...
  m_input->when(
    FL_WHEN_ENTER_KEY_ALWAYS); // callback triggered when user hits Enter
  m_input->maximum_size(1);
  m_input->color(COLOR_SELECTED_CELL);
}

void
LABSoft_GUI_LABChecker_Digital_Input_Table::event_callback2()
{
  int R = callback_row();
  int C = callback_col();

  LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext context =
    callback_context();

  switch (context) {
    case (
      LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::CONTEXT_CELL): {
      switch (Fl::event()) {
        case (FL_PUSH): {
          done_editing();
          start_editing(R, C);
    ...
    case (
      LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::CONTEXT_TABLE):
    case (LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::
            CONTEXT_ROW_HEADER):
    case (LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::
            CONTEXT_COL_HEADER): {
      done_editing();
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::input_cb2()
{
  set_value_hide();

  if (Fl::event_key() == FL_Enter) {
    if (static_cast<int>(m_col_edit) < (cols() - 1)) {
      start_editing(m_row_edit, m_col_edit + 1);
    } else {
      if (static_cast<int>(m_row_edit) < (rows() - 1)) {
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::recalculate_rows_and_cols()
{
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::recalculate_and_resize()
{
  recalculate_rows_and_cols();
  do_resize();
}
...
char
LABSoft_GUI_LABChecker_Digital_Input_Table::get_vector_value(unsigned R,
                                                             unsigned C) const
{
  if (C < m_input_bits) {
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::set_vector_value(char c,
                                                             unsigned R,
                                                             unsigned C)
{
  if (C < m_input_bits) {
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::event_callback(Fl_Widget* w,
                                                           void* v)
{
  (static_cast<LABSoft_GUI_LABChecker_Digital_Input_Table*>(v))
    ->event_callback2();
}

void
LABSoft_GUI_LABChecker_Digital_Input_Table::input_cb(Fl_Widget* w, void* v)
{
  (static_cast<LABSoft_GUI_LABChecker_Digital_Input_Table*>(v))->input_cb2();
}

void
LABSoft_GUI_LABChecker_Digital_Input_Table::do_resize()
{
  // 1. resize 2D vector
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::set_value_hide()
{
  std::string str(m_input->value());

  if (str == "0" || str == "1" || str == "X" || str == "x" || str == "`" ||
      str == "3") {
    if (str == "`") {
      str = "X";
    }
...
// row/column Preload the widget with the cell's current value, and make the
// widget 'appear' at the cell's location.
void
LABSoft_GUI_LABChecker_Digital_Input_Table::start_editing(int R, int C)
{
  m_row_edit = R;
  m_col_edit = C;
  ...
  m_input->value(s);
  m_input->position(0, static_cast<int>(strlen(s)));
  m_input->show();
  m_input->take_focus();
}
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::done_editing()
{
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::draw_cell(
  LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext context,
  int R,
  int C,
  int X,
  ...
  int H)
{
  ...
  switch (context) {
    ...
    case (LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::
      ...
      fl_push_clip(X, Y, W, H);
      {
        if (C < static_cast<int>(m_input_bits)) {
    ...
    case (
      LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::CONTEXT_CELL): {
      if (R == static_cast<int>(m_row_edit) &&
          C == static_cast<int>(m_col_edit) && m_input->visible()) {
        return;
      }
      ...
      fl_push_clip(X, Y, W, H);
      {
        ...
        fl_draw_box(FL_THIN_DOWN_BOX,
                    X,
                    Y,
                    W,
                    H,
                    is_selected(R, C) ? COLOR_SELECTED_CELL : FL_WHITE);
        fl_color(FL_BLACK);
    ...
    case (LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::
            CONTEXT_RC_RESIZE): {
      if (m_input->visible()) {
        find_cell(LABSoft_GUI_LABChecker_Digital_Input_Table::TableContext::
                    CONTEXT_TABLE,
                  m_row_edit,
                  m_col_edit,
                  X,
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::resize(unsigned new_number_of_rows,
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::input_bits(unsigned value)
{
  m_input_bits = value;
  m_max_output_count = static_cast<unsigned>(std::pow(2, value));
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::output_bits(unsigned value)
{
  m_output_bits = value;
...
void
LABSoft_GUI_LABChecker_Digital_Input_Table::output_count(unsigned value)
{
  if (value >= 1) {
...
unsigned
LABSoft_GUI_LABChecker_Digital_Input_Table::input_bits() const
{
  return (m_input_bits);
}

unsigned
LABSoft_GUI_LABChecker_Digital_Input_Table::output_count() const
{
  return (m_output_count);
}

unsigned
LABSoft_GUI_LABChecker_Digital_Input_Table::max_output_count() const
{
  return (m_max_output_count);
}

std::vector<std::vector<char>>&
LABSoft_GUI_LABChecker_Digital_Input_Table::inputs()
{
  return (m_inputs);
}

std::vector<std::vector<char>>&
LABSoft_GUI_LABChecker_Digital_Input_Table::outputs()
{
  return (m_outputs);
}

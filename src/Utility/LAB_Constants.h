...
constexpr float DISPLAY_UPDATE_RATE = 1.0f / 15.0f; // ideally 20 FPS
...
constexpr const char* DEFAULT_CALIBRATION_FILE_PATH =
  "../data/default.labcalibration";
constexpr const char* FACTORY_CALIBRATION_FILE_PATH =
  "../data/FACTORY.labcalibration";
}
...
namespace DMA {
namespace TI {
constexpr uint32_t OSC_PWM_PACING =
  (AP::DMA::TI_DATA::PERMAP(AP::DMA::PERIPH_DREQ::PWM)) |
  AP::DMA::TI_DATA::DEST_DREQ | AP::DMA::TI_DATA::WAIT_RESP |
  AP::DMA::TI_DATA::INTEN;

constexpr uint32_t OSC_TX =
  (AP::DMA::TI_DATA::PERMAP(AP::DMA::PERIPH_DREQ::SPI_TX)) |
  AP::DMA::TI_DATA::DEST_DREQ | AP::DMA::TI_DATA::WAIT_RESP |
  AP::DMA::TI_DATA::SRC_INC | AP::DMA::TI_DATA::INTEN;

constexpr uint32_t OSC_RX =
  (AP::DMA::TI_DATA::PERMAP(AP::DMA::PERIPH_DREQ::SPI_RX)) |
  AP::DMA::TI_DATA::SRC_DREQ | AP::DMA::TI_DATA::DEST_INC |
  AP::DMA::TI_DATA::WAIT_RESP;

constexpr uint32_t LOGAN_STORE =
  (AP::DMA::TI_DATA::PERMAP(AP::DMA::PERIPH_DREQ::PWM)) |
  AP::DMA::TI_DATA::SRC_DREQ | AP::DMA::TI_DATA::DEST_INC |
  AP::DMA::TI_DATA::WAIT_RESP;
};
...
constexpr unsigned VC_MEM_SIZE =
  AP::RPI::PAGE_SIZE + (NUMBER_OF_CHANNELS * NUMBER_OF_BUFFERS * BUFFER_SIZE);
...
constexpr uint32_t RAW_DATA_BIT_SHIFT_COUNT =
  (SAMPLE_SIZE * 8) / NUMBER_OF_CHANNELS;
constexpr uint32_t RAW_DATA_POST_SHIFT_MASK =
  (1 << RAW_DATA_BIT_SHIFT_COUNT) - 1;
...
constexpr double MAX_VERTICAL_OFFSET =
  MAX_VOLTAGE_PER_DIVISION * (DISPLAY_NUMBER_OF_ROWS / 2.0);       // 25 V
...
constexpr double MAX_TIME_PER_DIVISION =
  MAX_SAMPLES / (MIN_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
constexpr double MAX_TIME_PER_DIVISION_RECORDING =
  MAX_SAMPLES_RECORDING / (MIN_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
constexpr double MIN_TIME_PER_DIVISION =
  MIN_SAMPLES_DISPLAY / (MAX_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
constexpr double MIN_TIME_PER_DIVISION_SCREEN = 1.0 / DISPLAY_NUMBER_OF_COLUMNS;
constexpr double MIN_TIME_PER_DIVISION_NO_ZOOM =
  MAX_SAMPLES / (MAX_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
...
constexpr int Y_LABEL_INTERSPACE =
  40; // spacing between columns of y-axis labels
constexpr int Y_LABEL_UNIT_LEFT_MARGIN = 10;
constexpr int Y_LABEL_UNIT_BOTTOM_MARGIN =
  17; // padding of voltage unit of y-axis labels from top of grid
...
constexpr double ROFF_RESISTANCE =
  static_cast<uint32_t>(MCP4XXX::RESISTANCE_VERSION::_103);
// total offset digi pot resistance
};

namespace LOGAN {
// General
constexpr unsigned NUMBER_OF_CHANNELS =
  sizeof(PIN::LOGAN) / sizeof(PIN::LOGAN[0]);
...
constexpr unsigned VC_MEM_SIZE =
  AP::RPI::PAGE_SIZE + (NUMBER_OF_CHANNELS * NUMBER_OF_BUFFERS * BUFFER_LENGTH);
...
constexpr double MAX_TIME_PER_DIVISION_RECORDING =
  MAX_SAMPLES_RECORDING / (MIN_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
constexpr double MIN_TIME_PER_DIVISION =
  MIN_SAMPLES / (MAX_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
constexpr double MIN_TIME_PER_DIVISION_SCREEN = 1.0 / DISPLAY_NUMBER_OF_COLUMNS;
constexpr double MIN_TIME_PER_DIVISION_NO_ZOOM =
  MAX_NUMBER_OF_SAMPLES / (MAX_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
constexpr double MAX_HORIZONTAL_OFFSET = MAX_SAMPLES / MIN_SAMPLING_RATE;
constexpr double MIN_HORIZONTAL_OFFSET = (-1) * MAX_HORIZONTAL_OFFSET;
...
// Display
constexpr double MIN_TIME_PER_DIV_NO_ZOOM =
  MAX_NUMBER_OF_SAMPLES / (MAX_SAMPLING_RATE * DISPLAY_NUMBER_OF_COLUMNS);
};
...
// multiplexer
// {A0, A1}
constexpr unsigned MUX[LABC::OSC::NUMBER_OF_CHANNELS][2] = { { 27, 22 },
                                                             { 23, 24 } };

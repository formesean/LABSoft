...
struct LAB_Calibration_Data_Oscilloscope
{
  ...
  double conversion_reference_voltage
      = LABD::OSC::CONVERSION_REFERENCE_VOLTAGE;
  double conversion_constant = LABD::OSC::CONVERSION_CONSTANT;

  std::unordered_map<LABE::OSC::SCALING, double> vertical_offset_corrector
      = LABD::OSC::VERTICAL_OFFSET_CORRECTOR;
  std::unordered_map<LABE::OSC::SCALING, double> scaling_corrector_to_unity
      = LABD::OSC::SCALING_CORRECTOR_TO_UNITY;
  std::unordered_map<LABE::OSC::SCALING, double> scaling_corrector_to_actual
      = LABD::OSC::SCALING_CORRECTOR_TO_ACTUAL;
};
...
public:
  ...
  std::array<uint32_t, LABC::OSC::NUMBER_OF_SAMPLES> raw_data_buffer;

  std::array<LAB_Channel_Data_Oscilloscope, LABC::OSC::NUMBER_OF_CHANNELS>
      channel_data;
  ...
  {
    std::array<std::array<uint32_t, LABC::OSC::NUMBER_OF_SAMPLES>,
...
struct LAB_Parent_Data_Voltmeter
{
  ...
  std::array<LAB_Channel_Data_Voltmeter, LABC::VOLTMETER::NUMBER_OF_CHANNELS>
      channel_data;
};
...
struct LAB_Parent_Data_Ohmmeter
{
  ...
  std::array<LAB_Channel_Data_Ohmmeter, LABC::OHMMETER::NUMBER_OF_CHANNELS>
      channel_data;
};
...
{
public:
  std::array<LAB_Channel_Data_Function_Generator,
             LABC::FUNC_GEN::NUMBER_OF_CHANNELS>
      channel_data;

public:
  bool
  has_enabled_channels ()
  {
...
struct LAB_Parent_Data_Logic_Analyzer
{
  ...
  std::array<uint32_t, LABC::LOGAN::MAX_NUMBER_OF_SAMPLES> raw_data_buffer;

  std::array<LAB_Channel_Data_Logic_Analyzer, LABC::LOGAN::NUMBER_OF_CHANNELS>
      channel_data;
  ...
  struct TriggerBuffers
  {
    ...
    std::array<uint32_t, LABC::OSC::NUMBER_OF_SAMPLES> assembled_frame;
  } trigger_buffers;
};
...
struct LAB_DMA_Data_Oscilloscope
{
  ...
  uint32_t spi_samp_size = 0, spi_cs = 0, spi_cs_fifo_reset = 0,
           pwm_duty_cycle = 0, txd = 0;
...
struct LAB_DMA_Data_Logic_Analyzer
{
  ...
  uint32_t buffer_ok_flag = 0x1, pwm_duty_cycle;
